function [solutionNoRes,solutionRes,info] = ...
    solve_secondary_objectives(solutionMin,path,cfg)
%SOLVE_SECONDARY_OBJECTIVES Build upper-time seeds for the time Pareto sweep.
% The main Pareto objectives are machining time and vibration. This
% function solves only the Nmax no-suppression reference and the Nmax
% minimum-vibration seed. The full front is generated over every integer N
% by solve_all_time_vibration_sweep.
Nmin=solutionMin.N;
...
if abs(lambda-1)>1e-12
    error('POP_MOO:TimeParetoRequiresPureVibration', ...
        ['time_sweep_lambda_vibration must equal 1 for a true ' ...
        'machining-time/vibration Pareto front.']);
end
specB=struct('stage','secondary','lambda_vibration',1, ...
    'lambda_straightness',0,'straightness_scale',baseStraight, ...
    'vibration_scale',baseVibration, ...
    'resonance_band_reference',bandReference.proxy, ...
    'resonance_band_reference_actual',bandReference.actual);
solutionRes=solve_fixed_Nvec(path,cfg,Nvec,solutionNoRes,specB);
if ~solutionRes.feasible
    error('POP_MOO:SecondaryB', ...
        'Upper-time minimum-vibration seed is infeasible: %s',solutionRes.status);
end
solutionRes.name=sprintf('T=%.3f s - Minimum vibration seed at %g Hz', ...
    solutionRes.T,solutionRes.model.mode(1).frequency);
fprintf('  Upper-time vibration seed feasible: Jvib %.6g\n', ...
    solutionRes.objectives.vibration);
